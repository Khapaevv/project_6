...
class Message(models.Model):
    message_theme = models.CharField(max_length=150, help_text='Введите тему сообщения', verbose_name='Тема сообщения')
    message_body = models.TextField(help_text='Введите текст сообщения', verbose_name='Содержание')
...
class Mailing(models.Model):
    first_date = models.DateTimeField(default=timezone.now, verbose_name="Дата и время начала рассылки",
                                          help_text="Введите дату и время начала рассылки", **NULLABLE)
    next_date = models.DateTimeField(verbose_name="Дата и время следующей отправки рассылки", **NULLABLE)
    last_date = models.DateTimeField(verbose_name="Дата и время окончания рассылки",
                                         help_text="Введите дату и время окончания рассылки", **NULLABLE)
    intervals = models.CharField(max_length=10, choices=PERIODICITY_CHOICES, verbose_name='Периодичность', **NULLABLE)
...
class MailingLog(models.Model):
    last_mailing = models.DateTimeField(verbose_name="Дата и время последней попытки", **NULLABLE)
